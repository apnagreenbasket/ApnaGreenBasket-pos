import { useCallback, useEffect, useState, useMemo } from "react";
import type {
  DiscountApproval,
  ManualBill,
  BillingDateRangeMode,
} from "@/types";
import type { DraftCartItem } from "../modals/CreateBillDrawer";
import { isAuthError, formatLocalDate } from "../adminUtils";

export type { BillingDateRangeMode };

type UseBillingManagementProps = {
  accessToken: string | null;
  authHeaders: Record<string, string> | null;
  apiRequest: <T>(endpoint: string, options?: RequestInit) => Promise<T>;
  setNotice: (msg: string | null) => void;
  setError: (msg: string | null) => void;
  onBillSettled?: () => void;
  enabled?: boolean;
};

export function useBillingManagement({
  accessToken,
  authHeaders,
  apiRequest,
  setNotice,
  setError,
  onBillSettled,
  enabled = true,
}: UseBillingManagementProps) {
  const [billsList, setBillsList] = useState<ManualBill[]>([]);
  const [pendingApprovals, setPendingApprovals] = useState<DiscountApproval[]>([]);
  const [pendingApprovalsCount, setPendingApprovalsCount] = useState<number>(0);
  const [isLoadingBilling, setIsLoadingBilling] = useState(false);
  const [billingStatusFilter, setBillingStatusFilter] = useState<string>("ALL");
  const [billingSearchQuery, setBillingSearchQuery] = useState<string>("");
  const [dateRangeMode, setDateRangeMode] = useState<BillingDateRangeMode>("today");
  const [customStartDate, setCustomStartDate] = useState<string>(formatLocalDate());
  const [customEndDate, setCustomEndDate] = useState<string>(formatLocalDate());

  const { computedStartDate, computedEndDate } = useMemo(() => {
    const today = new Date();
    let start = new Date(today);
    let end = new Date(today);

    if (dateRangeMode === "today") {
      // already today
    } else if (dateRangeMode === "yesterday") {
      start.setDate(today.getDate() - 1);
      end.setDate(today.getDate() - 1);
    } else if (dateRangeMode === "last2days") {
      start.setDate(today.getDate() - 1);
    } else if (dateRangeMode === "week") {
      const day = today.getDay();
      const diff = (day === 0 ? -6 : 1) - day;
      start.setDate(today.getDate() + diff);
    } else if (dateRangeMode === "last7" || (dateRangeMode as string) === "last_7") {
      start.setDate(today.getDate() - 6);
    } else if (dateRangeMode === "this_month" || (dateRangeMode as string) === "thisMonth") {
      start = new Date(today.getFullYear(), today.getMonth(), 1);
    } else if (dateRangeMode === "last30" || (dateRangeMode as string) === "last_30") {
      start.setDate(today.getDate() - 29);
    } else if (dateRangeMode === "custom") {
      return { computedStartDate: customStartDate, computedEndDate: customEndDate };
    }
    
    return {
      computedStartDate: formatLocalDate(start),
      computedEndDate: formatLocalDate(end)
    };
  }, [dateRangeMode, customStartDate, customEndDate]);

  const {
    dailyGrandTotal,
    dailyNetPaid,
    dailyUpiPaid,
    dailyNetCash,
    dailyReturnsTotal,
    dailyCreditDebitNet,
    dailyCreditApplied,
    dailyDebitApplied,
    dailyDebtSettled,
    dailyCreditAwarded,
    dailyCreditCashedOut,
    dailyLoyaltyRedeemed,
  } = useMemo(() => {
    let grandTotal = 0;
    let netPaid = 0;
    let upiPaid = 0;
    let returnsTotal = 0;
    let creditApplied = 0;
    let debitApplied = 0;
    let debtSettled = 0;
    let creditAwarded = 0;
    let creditCashedOut = 0;
    let loyaltyRedeemed = 0;

    billsList.forEach((b) => {
      // Exclude exchange orders generated by the returns desk to keep POS sales pure
      const isExchange = (b.source || "").toUpperCase() === "EXCHANGE";
      if (isExchange) return;

      const s = (b.status || "").toUpperCase();
      if (s === "PAID" || s === "SERVED" || s === "COMPLETED" || s === "FINALIZED" || s === "PARTIALLY_REFUNDED") {
        grandTotal += b.total_amount || 0;
        const cApp = Number((b as any).credit_applied || 0);
        const dApp = Number((b as any).debit_applied || 0);
        const dSet = Number((b as any).debt_settled || 0);
        const cAwa = Number((b as any).credit_awarded || 0);
        const cCash = Number((b as any).credit_cashed_out || 0);
        const lRed = Number((b as any).loyalty_discount_inr || 0);

        creditApplied += cApp;
        debitApplied += dApp;
        debtSettled += dSet;
        creditAwarded += cAwa;
        creditCashedOut += cCash;
        loyaltyRedeemed += lRed;

        const net = (b.total_amount || 0)
                  - lRed
                  - cApp
                  - dApp
                  + dSet
                  + cAwa
                  - cCash;
        netPaid += net;

        const explicitUpi = Number((b as any).upi_amount || 0);
        if (explicitUpi > 0) {
          upiPaid += explicitUpi;
        } else if ((b.payment_method || "").toUpperCase() === "UPI") {
          upiPaid += Math.max(0, net);
        }
      }
    });

    const creditDebitNet = debtSettled + creditAwarded - creditApplied - debitApplied - creditCashedOut;
    const netCash = netPaid - upiPaid;

    return {
      dailyGrandTotal: grandTotal,
      dailyNetPaid: netPaid,
      dailyUpiPaid: upiPaid,
      dailyNetCash: netCash,
      dailyReturnsTotal: returnsTotal,
      dailyCreditDebitNet: creditDebitNet,
      dailyCreditApplied: creditApplied,
      dailyDebitApplied: debitApplied,
      dailyDebtSettled: debtSettled,
      dailyCreditAwarded: creditAwarded,
      dailyCreditCashedOut: creditCashedOut,
      dailyLoyaltyRedeemed: loyaltyRedeemed,
    };
  }, [billsList]);

  // Create Bill Modal State
  const [createBillModalOpen, setCreateBillModalOpen] = useState(false);
  const [editingDraftBillId, setEditingDraftBillId] = useState<string | null>(null);
  const [editingCompletedBill, setEditingCompletedBill] = useState<ManualBill | null>(null);
  const [selectedTable, setSelectedTable] = useState("WALK-IN");
  const [customerName, setCustomerName] = useState("");
  const [customerPhone, setCustomerPhone] = useState("");
  const [customerExtraDetail, setCustomerExtraDetail] = useState("");
  const [deliveryCharge, setDeliveryCharge] = useState<number>(0);
  const [handlingCharge, setHandlingCharge] = useState<number>(0);
  const [draftCartItems, setDraftCartItems] = useState<DraftCartItem[]>([]);
  const [isInterstate, setIsInterstate] = useState<boolean>(false);
  const [placeOfSupply, setPlaceOfSupply] = useState<string>("");
  const [customerGstin, setCustomerGstin] = useState<string>("");
  const [customerLegalName, setCustomerLegalName] = useState("");
  const [isWalkIn, setIsWalkIn] = useState<boolean>(false);

  // Discount Modal State
  const [discountModalOpen, setDiscountModalOpen] = useState(false);
  const [discountTargetBill, setDiscountTargetBill] = useState<ManualBill | null>(null);
  const [discountType, setDiscountType] = useState<"PERCENT" | "FLAT" | "COMPLIMENTARY">("PERCENT");
  const [discountValue, setDiscountValue] = useState<number>(0);
  const [discountReason, setDiscountReason] = useState<string>("");

  // Payment Modal State
  const [paymentModalOpen, setPaymentModalOpen] = useState(false);
  const [paymentTargetBill, setPaymentTargetBill] = useState<ManualBill | null>(null);
  const [selectedPaymentMethod, setSelectedPaymentMethod] = useState<"CASH" | "UPI">("CASH");
  const [cashTendered, setCashTendered] = useState<string>("");

  // Load Billing Data
  const loadBillingData = useCallback(async () => {
    if (!authHeaders || !enabled) return;
    setIsLoadingBilling(true);
    try {
      const [billsRes, approvalsRes, countRes] = await Promise.all([
        apiRequest<ManualBill[]>(`/api/billing/bills?start_date=${computedStartDate}&end_date=${computedEndDate}`).catch(() => []),
        apiRequest<DiscountApproval[]>("/api/billing/pending-approvals").catch(() => []),
        apiRequest<{ count: number }>("/api/billing/pending-approvals-count").catch(() => ({ count: 0 })),
      ]);
      setBillsList(billsRes || []);
      setPendingApprovals(approvalsRes || []);
      setPendingApprovalsCount(countRes?.count || 0);
    } catch (err) {
      if (isAuthError(err)) return;
      console.error("Billing fetch error:", err);
    } finally {
      setIsLoadingBilling(false);
    }
  }, [apiRequest, authHeaders, computedStartDate, computedEndDate, enabled]);

  // Auto-load billing data when authHeaders is ready and billing is enabled
  useEffect(() => {
    if (authHeaders && enabled) {
      void loadBillingData();
    }
  }, [authHeaders, loadBillingData, enabled]);

  const handleCreateBill = async (proceedToPayment = false) => {
    if (!draftCartItems.length) {
      setError("Please add at least one menu item to the bill.");
      return;
    }
    if (proceedToPayment) {
      if (!isWalkIn) {
        const cleanPhone = customerPhone.replace(/\D/g, "");
        if (!cleanPhone || cleanPhone.length < 10) {
          setError("Customer phone number must be at least 10 digits, or check 'Walk in'.");
          return;
        }
      }
    } else {
      if (customerPhone.trim()) {
        const cleanPhone = customerPhone.replace(/\D/g, "");
        if (cleanPhone.length > 0 && cleanPhone.length < 10 && !isWalkIn) {
          setError("Customer phone number must be at least 10 digits");
          return;
        }
      }
    }
    try {
      const payload = {
        basket_number: selectedTable || "WALK-IN",
        customer_name: customerName || null,
        customer_phone: customerPhone || null,
        customer_extra_detail: customerExtraDetail || null,
        is_interstate: isInterstate,
        place_of_supply: placeOfSupply || null,
        customer_gstin: customerGstin || null,
        customer_legal_name: customerLegalName || null,
        items: draftCartItems.map((item) => ({
          menu_item_id: item.menu_item_id || null,
          variant_id: item.variant_id || null,
          selected_batch_id: item.selected_batch_id || null,
          allow_oversell: !!item.allow_oversell,
          item_name: item.item_name,
          quantity: item.quantity,
          unit_price: item.unit_price,
          mrp: item.mrp || null,
          tax_rate: item.tax_rate || null,
          hsn_code: item.hsn_code || null,
          pricing_type: item.pricing_type || "RETAIL",
          is_complimentary: item.is_complimentary,
          selected_unit: item.selected_unit || null,
        })),
        ...(editingCompletedBill ? { replaces_bill_id: editingCompletedBill.id } : {})
      };

      let billResult: ManualBill;
      if (editingDraftBillId) {
        // Update existing draft (no duplicate draft created!)
        billResult = await apiRequest<ManualBill>(`/api/billing/bills/${editingDraftBillId}`, {
          method: "PUT",
          body: JSON.stringify(payload),
        });
      } else {
        // Create new draft bill
        billResult = await apiRequest<ManualBill>("/api/billing/bills", {
          method: "POST",
          body: JSON.stringify(payload),
        });
      }

      setCreateBillModalOpen(false);
      void loadBillingData();

      if (proceedToPayment && billResult) {
        setEditingDraftBillId(billResult.id);
        setPaymentTargetBill(billResult);
        setCashTendered("");
        setSelectedPaymentMethod("CASH");
        setPaymentModalOpen(true);
      } else {
        resetCreateBillForm();
        setNotice(editingDraftBillId ? "Draft bill updated!" : "Manual bill created successfully!");
      }
    } catch (err) {
      setError(err instanceof Error ? err.message : "Failed to save bill.");
    }
  };

  const handleResumeDraft = (bill: ManualBill) => {
    setEditingDraftBillId(bill.id);
    if (bill.replaces_bill_id) {
      const orig = billsList.find((b) => b.id === bill.replaces_bill_id);
      if (orig) {
        setEditingCompletedBill(orig);
      } else {
        void apiRequest<ManualBill>(`/api/billing/bills/${bill.replaces_bill_id}`)
          .then((res) => setEditingCompletedBill(res))
          .catch(() => setEditingCompletedBill(null));
      }
    } else {
      setEditingCompletedBill(null);
    }
    setSelectedTable(bill.basket_number || "WALK-IN");

    setCustomerName(bill.customer_name || "");
    setCustomerPhone(bill.customer_phone || "");
    setIsWalkIn(!bill.customer_phone);
    setIsInterstate(Boolean((bill as any).is_interstate));
    setPlaceOfSupply((bill as any).place_of_supply || "");
    setCustomerGstin((bill as any).customer_gstin || "");
    setCustomerLegalName((bill as any).customer_legal_name || "");
    setDeliveryCharge(bill.delivery_charge || 0);
    setHandlingCharge(bill.handling_charge || 0);
    if (bill.items && bill.items.length > 0) {
      const itemsMapped: DraftCartItem[] = bill.items.map((it: any) => {
        const uPrice = typeof it.unit_price === "number" ? it.unit_price : parseFloat(it.unit_price) || 0;
        const uMrp = typeof it.mrp === "number" ? it.mrp : parseFloat(it.mrp) || null;
        return {
          menu_item_id: it.menu_item_id || "",
          variant_id: it.variant_id || null,
          selected_batch_id: it.selected_batch_id || null,
          selected_batch_number: it.selected_batch_number || null,
          item_name: it.item_name || it.menu_item?.name || "Item",
          unit_price: uPrice,
          mrp: uMrp,
          tax_rate: typeof it.tax_rate === "number" ? it.tax_rate : parseFloat(it.tax_rate) || null,
          hsn_code: it.hsn_code || (it.menu_item && it.menu_item.hsn_code) || null,
          quantity: typeof it.quantity === "number" ? it.quantity : parseFloat(it.quantity) || 1,
          pricing_type: "RETAIL",
          is_complimentary: !!it.is_complimentary,
          selected_unit: it.selected_unit || it.unit || null,
          base_unit_price: typeof it.base_unit_price === "number" ? it.base_unit_price : (parseFloat(it.base_unit_price) || uPrice),
          base_mrp: typeof it.base_mrp === "number" ? it.base_mrp : (parseFloat(it.base_mrp) || uMrp),
          is_custom_price: !!it.is_custom_price,
        };
      });
      setDraftCartItems(itemsMapped);
    } else {
      setDraftCartItems([]);
    }
    setCreateBillModalOpen(true);
  };

  const handleEditCompletedBill = (bill: ManualBill) => {
    setEditingCompletedBill(bill);
    setSelectedTable(bill.basket_number || "WALK-IN");
    setCustomerName(bill.customer_name || "");
    setCustomerPhone(bill.customer_phone || "");
    setIsWalkIn(!bill.customer_phone);
    setIsInterstate(Boolean((bill as any).is_interstate));
    setPlaceOfSupply((bill as any).place_of_supply || "");
    setCustomerGstin((bill as any).customer_gstin || "");
    setCustomerLegalName((bill as any).customer_legal_name || "");
    setDeliveryCharge(bill.delivery_charge || 0);
    setHandlingCharge(bill.handling_charge || 0);
    if (bill.items && bill.items.length > 0) {
      const itemsMapped: DraftCartItem[] = bill.items.map((it: any) => {
        const uPrice = typeof it.unit_price === "number" ? it.unit_price : parseFloat(it.unit_price) || 0;
        const uMrp = typeof it.mrp === "number" ? it.mrp : parseFloat(it.mrp) || null;
        return {
          menu_item_id: it.menu_item_id || "",
          variant_id: it.variant_id || null,
          selected_batch_id: it.selected_batch_id || null,
          selected_batch_number: it.selected_batch_number || null,
          item_name: it.item_name || it.menu_item?.name || "Item",
          unit_price: uPrice,
          mrp: uMrp,
          tax_rate: typeof it.tax_rate === "number" ? it.tax_rate : parseFloat(it.tax_rate) || null,
          hsn_code: it.hsn_code || (it.menu_item && it.menu_item.hsn_code) || null,
          quantity: typeof it.quantity === "number" ? it.quantity : parseFloat(it.quantity) || 1,
          pricing_type: "RETAIL",
          is_complimentary: !!it.is_complimentary,
          selected_unit: it.selected_unit || it.unit || null,
          base_unit_price: uPrice,
          base_mrp: uMrp,
        };
      });
      setDraftCartItems(itemsMapped);
    } else {
      setDraftCartItems([]);
    }
    setCreateBillModalOpen(true);
  };


  const handleApplyDiscount = async (itemComplimentaryQuantities?: Record<string, number>, isItemLevelComp?: boolean) => {
    if (!discountTargetBill) return;
    if (discountType !== "COMPLIMENTARY" && (!discountReason || discountReason.trim().length < 2)) {
      setError("Please provide a reason note for the discount.");
      return;
    }

    try {
      const res = await apiRequest<ManualBill>(`/api/billing/bills/${discountTargetBill.id}/apply-discount`, {
        method: "POST",
        body: JSON.stringify({
          discount_type: isItemLevelComp ? "COMPLIMENTARY_ITEMS" : discountType,
          discount_value: discountValue,
          reason_note: discountReason || "Complimentary items",
          item_complimentary_quantities: itemComplimentaryQuantities || null,
        }),
      });
      if (res.discount_status === "PENDING_APPROVAL") {
        setNotice("Discount requested! Flagged for Manager Approval.");
      } else {
        setNotice(`Discount applied to Bill #${discountTargetBill.id.slice(0, 8).toUpperCase()}!`);
      }
      setDiscountModalOpen(false);
      setDiscountTargetBill(null);
      setDiscountReason("");
      setDiscountValue(0);
      if (res) {
        setBillsList((prev) => prev.map((b) => (b.id === res.id ? res : b)));
        if (paymentTargetBill && paymentTargetBill.id === res.id) {
          setPaymentTargetBill(res);
        }
      }
      void loadBillingData();
    } catch (err) {
      setError(err instanceof Error ? err.message : "Failed to apply discount.");
    }
  };

  const handleResolveApproval = async (approvalId: string, approve: boolean) => {
    try {
      await apiRequest(`/api/billing/approvals/${approvalId}/resolve`, {
        method: "POST",
        body: JSON.stringify({ approve }),
      });
      setNotice(`Discount approval ${approve ? "APPROVED" : "REJECTED"}.`);
      void loadBillingData();
    } catch (err) {
      setError(err instanceof Error ? err.message : "Failed to resolve approval.");
    }
  };

  const handleMarkPaid = async (
    cashDenominations?: Record<string, number>,
    changeDenominations?: Record<string, number>,
    redeemLoyaltyPoints?: number,
    deliveryCharge?: number,
    handlingCharge?: number,
    applyCredit?: number,
    recordDebit?: number,
    recordCredit?: number,
    debtSettled?: number,
    creditCashedOut?: number,
    cashAmount?: number,
    upiAmount?: number
  ) => {
    if (!paymentTargetBill) return;
    try {
      const isSplit = (cashAmount !== undefined && upiAmount !== undefined && cashAmount > 0 && upiAmount > 0);
      const pm = isSplit ? "SPLIT" : selectedPaymentMethod;

      await apiRequest<ManualBill>(`/api/billing/bills/${paymentTargetBill.id}/mark-paid`, {
        method: "POST",
        body: JSON.stringify({
          payment_method: pm,
          cash_amount: cashAmount,
          upi_amount: upiAmount,
          cash_denominations: cashDenominations || null,
          change_denominations: changeDenominations || null,
          redeem_loyalty_points: redeemLoyaltyPoints || 0,
          delivery_charge: deliveryCharge || 0,
          handling_charge: handlingCharge || 0,
          apply_credit: applyCredit || 0,
          record_debit: recordDebit || 0,
          record_credit: recordCredit || 0,
          debt_settled: debtSettled || 0,
          credit_cashed_out: creditCashedOut || 0,
        }),
      });
      setPaymentModalOpen(false);
      setPaymentTargetBill(null);
      setCashTendered("");
      resetCreateBillForm();
      setNotice(`Bill #${paymentTargetBill.id.slice(0, 8).toUpperCase()} marked as PAID via ${pm}! Stock auto-deducted.`);
      void loadBillingData();
      onBillSettled?.();
    } catch (err) {
      setError(err instanceof Error ? err.message : "Failed to mark bill paid.");
    }
  };

  const handleDeleteBill = async (billId: string) => {
    setError(null);
    try {
      await apiRequest(`/api/billing/bills/${billId}`, { method: "DELETE" });
      setBillsList((current) => current.filter((b) => b.id !== billId));
      setNotice(`Bill #${billId.slice(0, 8)} deleted permanently.`);
      onBillSettled?.();
    } catch (err) {
      setError(err instanceof Error ? err.message : "Delete failed.");
      throw err;
    }
  };

  const openDiscountModal = (bill: ManualBill) => {
    setDiscountTargetBill(bill);
    setDiscountType("PERCENT");
    setDiscountValue(0);
    setDiscountReason("");
    setDiscountModalOpen(true);
  };

  const openPaymentModal = (bill: ManualBill) => {
    setPaymentTargetBill(bill);
    if (bill.replaces_bill_id) {
      const orig = billsList.find((b) => b.id === bill.replaces_bill_id);
      if (orig) {
        setEditingCompletedBill(orig);
      } else {
        void apiRequest<ManualBill>(`/api/billing/bills/${bill.replaces_bill_id}`)
          .then((res) => setEditingCompletedBill(res))
          .catch(() => setEditingCompletedBill(null));
      }
    } else {
      setEditingCompletedBill(null);
    }
    setSelectedPaymentMethod("CASH");
    setCashTendered("");
    setPaymentModalOpen(true);
  };


  const resetCreateBillForm = useCallback(() => {
    setEditingDraftBillId(null);
    setEditingCompletedBill(null);
    setSelectedTable("WALK-IN");
    setCustomerName("");
    setCustomerPhone("");
    setCustomerExtraDetail("");
    setIsInterstate(false);
    setPlaceOfSupply("");
    setCustomerGstin("");
    setCustomerLegalName("");
    setDeliveryCharge(0);
    setHandlingCharge(0);
    setDraftCartItems([]);
    setIsWalkIn(false);
  }, []);

  const handleCloseCreateBillDrawer = useCallback(() => {
    setCreateBillModalOpen(false);
    resetCreateBillForm();
  }, [resetCreateBillForm]);

  const handleKeepAsDraft = async () => {
    if (!paymentTargetBill) return;
    setNotice(`Bill #${paymentTargetBill.id.slice(0, 8).toUpperCase()} saved as draft.`);
    setPaymentModalOpen(false);
    setPaymentTargetBill(null);
    setCashTendered("");
    resetCreateBillForm();
    void loadBillingData();
  };

  const handleDiscardPaymentBill = async () => {
    if (paymentTargetBill) {
      try {
        if (paymentTargetBill.status !== "PAID" && paymentTargetBill.status !== "COMPLETED") {
          await apiRequest(`/api/billing/bills/${paymentTargetBill.id}`, {
            method: "DELETE",
          });
        }
      } catch (err) {
        console.error("Error deleting discarded bill:", err);
      }
    }
    setPaymentModalOpen(false);
    setPaymentTargetBill(null);
    setCashTendered("");
    setCreateBillModalOpen(false);
    resetCreateBillForm();
    void loadBillingData();
  };

  const handleBackToDrawer = useCallback(() => {
    setPaymentModalOpen(false);
    setCreateBillModalOpen(true);
  }, []);

  return {
    billsList,
    openDiscountModal,
    openPaymentModal,
    editingCompletedBill,
    setEditingCompletedBill,
    handleEditCompletedBill,
    handleKeepAsDraft,
    handleDiscardPaymentBill,
    handleBackToDrawer,
    setBillsList,
    pendingApprovals,
    setPendingApprovals,
    pendingApprovalsCount,
    setPendingApprovalsCount,
    isLoadingBilling,
    billingStatusFilter,
    setBillingStatusFilter,
    billingSearchQuery,
    setBillingSearchQuery,
    createBillModalOpen,
    setCreateBillModalOpen,
    selectedTable,
    setSelectedTable,
    customerName,
    setCustomerName,
    customerPhone,
    setCustomerPhone,
    isWalkIn,
    setIsWalkIn,
    customerExtraDetail,
    setCustomerExtraDetail,
    isInterstate,
    setIsInterstate,
    placeOfSupply,
    setPlaceOfSupply,
    customerGstin,
    setCustomerGstin,
    customerLegalName,
    setCustomerLegalName,
    deliveryCharge,
    setDeliveryCharge,
    handlingCharge,
    setHandlingCharge,
    draftCartItems,
    setDraftCartItems,
    discountModalOpen,
    setDiscountModalOpen,
    discountTargetBill,
    setDiscountTargetBill,
    discountType,
    setDiscountType,
    discountValue,
    setDiscountValue,
    discountReason,
    setDiscountReason,
    paymentModalOpen,
    setPaymentModalOpen,
    paymentTargetBill,
    setPaymentTargetBill,
    selectedPaymentMethod,
    setSelectedPaymentMethod,
    cashTendered,
    setCashTendered,
    loadBillingData,
    handleCreateBill,
    handleResumeDraft,
    handleCloseCreateBillDrawer,
    resetCreateBillForm,
    handleApplyDiscount,
    handleResolveApproval,
    handleMarkPaid,
    handleDeleteBill,
    dateRangeMode,
    setDateRangeMode,
    customStartDate,
    setCustomStartDate,
    customEndDate,
    setCustomEndDate,
    computedStartDate,
    computedEndDate,
    dailyGrandTotal,
    dailyNetPaid,
    dailyUpiPaid,
    dailyNetCash,
    dailyReturnsTotal,
    dailyCreditDebitNet,
    dailyCreditApplied,
    dailyDebitApplied,
    dailyDebtSettled,
    dailyCreditAwarded,
    dailyCreditCashedOut,
    dailyLoyaltyRedeemed,
  };
}
